...
//路徑及帳號控管
	...
	$USER_ID = $_SESSION["C_USER_ID"];														//管理員 Account
...
//搜尋條件
	if($sNM != ""){
		$PageCond .= "&sNM=" . $sNM;
		$myWhere .= " and FullName like '%" . $sNM . "%' ";
	}
//主檔	: 最新消息
	$Sql = " 
		Select
			Id, FullName, Tel1, Tel2, CreatorId, LastEditorId, LastEditDate, Enabled
		From Personnel
	";
	$Sql .= $myWhere;
...
//基礎授權
	$Sql = " select TeacherNumber from License ";
	$Sql .= " where 1 = 1 ";
...
if($CountCH > $TeacherCount){
	
	$Num = $CountCH - $TeacherCount;
	
	$Sql = " Update Personnel set Enabled = 'N' ";
	$Sql .= " where 1 = 1 ";
	$Sql .= " and IdentityTypeId = '000000000000002' ";
	$Sql .= " and ClientId = '".$USER_NUM."' ";
	$Sql .= " and Enabled = 'Y' ";
...
<title>Web_園方 教師 自購家長(管理者介面) 教師與課表管理 本功能第一頁(首頁)</title>
...
<?php include_once(root_path . "/SystemMaintain/CommonPage/header_system_helper.php");?>                 
    ...
    <div class="doc">
        ...
        <div class="mainOptionTool">
            <ul>
                <li class="title"><span class="sized-text-normal">教師姓名</span> <input type="text" name="sNM" id="sNM" class="search sized-text-normal"></li>
                <li class="find"><button type="button"><span id="TxtTipBtnSrh" class="sized-text-normal">搜尋</span></button></li>
                <li class="new"><button type="button" onClick="Excute('Add');"><span class="sized-text-normal">新增教師</span></button></li>
            </ul>
            <div class="clearFloat"></div>
        </div>
        <!-- 資料管理操作 end -->
        	...
        	<table cellpadding="0" cellspacing="0" class="topAndLeftHeader">
             <form name="DataForm" id="DataForm" method="post">
            	...
            	<tr>
                    ...
                    <th class="topHeader"><span class="hor-box-text-large">連絡電話1</span></th>
                    <th class="topHeader"><span class="hor-box-text-large">連絡電話2</span></th>
                ...
                <tr <?php if($lineCount % 2 == 1){ echo ' class="odd"';}else{ echo ' class="even"' ;}?>>
                	<th valign="middle">
                    	<ul class="operations">
                            ...
                            <?php if($CountCH < $TeacherCount){?>
                            <li><a href="#" onClick="SetKey('<?php echo $initAry[0];?>');<?php if($initAry[7] == "Y"){ ?>Excute('Close');<?php ?><?php }else{?>Excute('Open');<?php }?>" <?php if($initAry[7] == "Y"){ echo 'class="check"'?><?php }else{ echo 'class="disable"'?><?php }?>><span class="hiddenItem">啟用</span></a></li>
                            <?php }else{?>
                            <li><a href="#" onClick="SetKey('<?php echo $initAry[0];?>');<?php if($initAry[7] == "Y"){ ?>Excute('Close');<?php ?><?php }else{?>alert('啟用人數超過限制');<?php }?>" <?php if($initAry[7] == "Y"){ echo 'class="check"'?><?php }else{ echo 'class="disable"'?><?php }?>><span class="hiddenItem">啟用</span></a></li>
                            <?php }?>
                        </ul>
                    </th>
                    ...
                    <td class="leftAlignText"><span class="hor-box-text-normal"><?php echo DspDate(substr($initAry[6],0,8),"/").' '. DspTime(substr($initAry[6],8,6),":")?></span></td>
                </tr>
                <?php }?>
            </form>
            </table>
