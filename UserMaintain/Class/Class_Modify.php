...
//
	...
	$StudAry = $MySql -> db_array($Sql,1);
	for($i=0;$i<count($StudAry);$i++){
		if($i<count($StudAry)){
			$Stu2 = $Stu2.$StudAry[$i]."','";
		}else{
			$Stu2 = '';
...
<title>Web_園方 教師 自購家長(管理者介面) 班級與學生管理 班級管理 編輯班級</title>
...
<?php include_once(root_path . "/SystemMaintain/CommonPage/header_topmenu_item_4.php");?>          
        ...
        <div class="breadCrumb">
            <ul>
                ...
                <li class="current"><span class="hor-box-text">編輯班級</span></li><!-- 如果沒有，就不寫入這個LI -->
        ...
        <div class="addSystemUser newClass">
            <table cellpadding="0" cellspacing="0" class="leftHeader">
                <tr>
                    ...
                    <td valign="middle"><input type="text" id="ClassName" name="ClassName" class="halfWidth sized-text-normal" placeholder="例：105 擴思訊之福祿貝爾1-10 20天班" value="<?php echo $rs["ClassName"];?>"></td>
                </tr>
                <tr>
                    ...
                    <td valign="middle">
                        ...
                        <?php for($i=0;$i<count($CLevelAry);$i++){?>
                        	<option value="<?php echo $CLevelAry[$i][0];?>" <?php if($rs["ClassLevelId"]==$CLevelAry[$i][0]){?>selected<?php }?>><?php echo $CLevelAry[$i][4];?></option>
                        <?php }?>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td valign="middle"><span id="Feq" name="Feq" class="hor-box-text-normal"><?php echo $rs["Comment"];?></span></td>
                </tr>
                <tr>
                    <th valign="top"><span class="hor-box-text-normal">排課開始時間：</span></th>
                    <td valign="middle"><input type="text" id="StartTime" name="StartTime" class="halfWidth sized-text-normal datepicker" readonly value="<?php echo $rs["StartTime"];?>"></td>
                </tr>
                <tr>
                    <th valign="top"><span class="hor-box-text-normal">上課日期：</span></th>
                    <td valign="middle">
                    	<label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="1" <?php if(strstr($Week,"1")){?>checked<?php }?>><span class="sized-text-1">週一</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="2" <?php if(strstr($Week,"2")){?>checked<?php }?>><span class="sized-text-1">週二</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="3" <?php if(strstr($Week,"3")){?>checked<?php }?>><span class="sized-text-1">週三</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="4" <?php if(strstr($Week,"4")){?>checked<?php }?>><span class="sized-text-1">週四</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="5" <?php if(strstr($Week,"5")){?>checked<?php }?>><span class="sized-text-1">週五</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="6" <?php if(strstr($Week,"6")){?>checked<?php }?>><span class="sized-text-1">週六</span></label>&nbsp;
                        <label><input type="checkbox" id="ClassWeek" name="ClassWeek" value="7" <?php if(strstr($Week,"7")){?>checked<?php }?>><span class="sized-text-1">週日</span></label>
                    </td>
                </tr>
                <tr>
