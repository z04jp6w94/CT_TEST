...
//驗證資料
		...
		if ($DataKey != '') {
			$Sql = " Select DeleteStatus From Class where Id = $DataKey ";
			$Focus = $MySql -> db_query($Sql) or die("查詢 Query 錯誤");
			$rs = $MySql -> db_fetch_array($Focus);
			
//更新狀態
			if ($rs["DeleteStatus"] != "Y"){
				$Sql = " Update Class Set DeleteStatus = 'Y' Where Id = $DataKey ";
				...
				$Sql = " Update Class Set DeleteStatus = 'N' Where Id = $DataKey ";	
