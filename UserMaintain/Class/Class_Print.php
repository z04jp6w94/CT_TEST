...
//取得班級學生	
	$Sql = " select M.*, A.Name FROM ClassMember M ";
	$Sql .= " left join Student A on A.Id = M.StudentId "; 
	$Sql .= " where 1 = 1 ";
	$Sql .= " and ClassId = '".$DataKey."' ";
	$CMRun = $MySql -> db_query($Sql) or die("查詢 Query 錯誤");
	$CMAry = $MySql -> db_array($Sql,10);
//取得亂數QRCODE
...
//動態產生QR_CODE
	for($i=0;$i<count($CMAry);$i++){
		 ${"QR_CODE".$i} = generateRandomString();
//		 echo ${"QR_CODE".$i};
	}	
//寫入 Authorization
	for($i=0;$i<count($CMAry);$i++){
		...
		$Sql .= " values";
		$Sql .= " ('', '".$CMAry[$i][2]."', 'Y', '".${"QR_CODE".$i}."', 'N', '".$ExID."', '".$ExDate.$ExTime."', '".$ExID."', '".$ExDate.$ExTime."', 'N' ) ";
...
1對多的體貼設計，爸爸、媽媽、爺爺、奶奶、外公、外婆等可同時安裝，一起來關心寶貝！
            ...
            <dl class="steps sp3">
                ...
                <dd>
                	<ul class="infoList">
                    	<!--<li class="pic"><img src="http://chart.googleapis.com/chart?cht=qr&chl=<?php echo ${"QR_CODE".$i};?>&chld=L|0&chs=30x30" border="0"></li>-->
                        <li class="text"><p>帳戶授權碼<br><?php echo ${"QR_CODE".$i};?></p></li>
                        <br class="clearFloat">
                    </ul>
                </dd>
            </dl>
