...
************************************************************************************************************/	
	...
	JSDragDropTree.prototype = {
		...
		,		
		setRootName : function(nameOfRoot)
		{
		...
		*/
		...
		{
			var message = '您是否確定要刪除[' + obj2.innerHTML+']';
			if(this.hasSubNodes(obj2.parentNode)) message = message + '\n\n[' + obj2.innerHTML+']下的子項目將自動轉移到['+this.nameOfRoot+']下！';
			if(confirm(message)){
		...
		{
			//新增資料夾時先檢查是否已達最大層數
			var _CurrLevel=JSTreeObj.dragDropCountLevels(obj2,'up');
			if(_CurrLevel>=JSTreeObj.maximumDepth){
				alert(JSTreeObj.messageMaximumDepthReached);
				return false;
			}else{
				this.__addItem_step2(obj2.parentNode);	// Sending <LI> tag to the __deleteItem_step2 method	
			}
		...
		{
			if(this.ajaxObjects[ajaxIndex].response!='SUCCESS'){
				alert('刪除資料夾時發生錯誤！:'); 	// delete failed
			}			
			ReloadPage();
		...
		{
			//新增一個目錄，一併儲存目前選單順序
			//var saveString = 'PTI='+obj.id.replace(/[^0-9]/gi,'')+',TT=D,GI='+this.idOfTree;
			var orderString = treeObj.getNodeOrders();
//			// Creating ajax object and send items
			var ajaxIndex = JSTreeObj.ajaxObjects.length;
			JSTreeObj.ajaxObjects[ajaxIndex] = new sack();
			JSTreeObj.ajaxObjects[ajaxIndex].method = "POST";
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("saveOrder", orderString);
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("addDir", obj.id.replace(/[^0-9]/gi,''));
			...
			JSTreeObj.ajaxObjects[ajaxIndex].onCompletion = function() { JSTreeObj.__addComplete(ajaxIndex,obj); } ;	// Specify function that will be executed after file has been found
			JSTreeObj.ajaxObjects[ajaxIndex].runAJAX();		// Execute AJAX function				
			
		}
		,
		__addComplete : function(ajaxIndex,obj)
		{
			if(this.ajaxObjects[ajaxIndex].response!='SUCCESS'){
				alert('新增資料夾時發生錯誤！'); 	// Add failed
			}			
			ReloadPage();
		}
		,
		...
		{
			if(this.ajaxObjects[ajaxIndex].response!='SUCCESS'){
				alert('更名資料夾時發生錯誤！'+this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}
			ReloadPage();
		}
		,
		...
		initTree : function()
		{
			...
			if(this.deleteAllowed || this.renameAllowed || this.addAllowed){
				try{
					...
					if(this.addAllowed)menuModel.addItem(1,'新增資料夾','','',false,'JSTreeObj.addItem');
					if(this.deleteAllowed)menuModel.addItem(2,'刪除資料夾','','',false,'JSTreeObj.deleteItem');
					if(this.renameAllowed)menuModel.addItem(3,'更名資料夾','','',false,'JSTreeObj.renameItem');
					menuModel.init();	

					var menuModelAddOnly = new DHTMLGoodies_menuModel();
					if(this.addAllowed)menuModelAddOnly.addItem(4,'新增資料夾','','',false,'JSTreeObj.addItem');
					menuModelAddOnly.init();	

					var menuModelRenameOnly = new DHTMLGoodies_menuModel();
					if(this.renameAllowed)menuModelRenameOnly.addItem(5,'刪除資料夾','','',false,'JSTreeObj.renameItem');
					menuModelRenameOnly.init();	
					
					var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(6,'更名資料夾','','',false,'JSTreeObj.deleteItem');
			...
			for(var no=0;no<menuItems.length;no++){
				...
				/*設定該項目所使用的圖示，若該項目有設定ICON="ImgFileName"時，則改以ImgFileName作為圖示名稱顯示。*/
				var spIcon=menuItems[no].getAttributeNode('icon');
				if(spIcon){
					spIcon=menuItems[no].icon;
					folderImg.src = this.imageFolder + spIcon;
				}else{
					/*修改成當該項目設為無子項時，使用文件圖示*/
					if (!noChildren){
				...
				if(this.contextMenu){
					...
					if(noRename=='true' && noDelete=='true' && noAdd=='true'){}else{
						if(noDelete == 'true' && noRename == 'true')this.contextMenu.attachToElement(aTag,false,menuModelAddOnly);
						else if(noDelete == 'true' && noAdd == 'true')this.contextMenu.attachToElement(aTag,false,menuModelRenameOnly);
						else if(noRename == 'true' && noAdd == 'true')this.contextMenu.attachToElement(aTag,false,menuModelDeleteOnly);
