...
/*
	...
	_data5	: 可傳遞參數 5
*/
	function LoadHtmlToId(url, id, _data1, _data2, _data3, _data4, _data5){
		$(id).load(
			url,
			{data1: _data1, data2: _data2, data3: _data3, data4: _data4, data5: _data5},
			function(){
			}
		);
	}
// 偵測 User Devices Orientation
	function DevicesOrientation(){
...
// 偵測 User Devices
	var isMobile = isUserAgentCheck(["iPhone", "android"]);
	var isIpad = isUserAgentCheck(["iPad"]);
	var isMobileVeryLow = isUserAgentCheck(["iPhone", "iPad", "android", "blackberry"]);
	function isUserAgentCheck(userAgents){
		var agent = navigator.userAgent;
		for(var i = 0; i < userAgents.length; i++)
		{
			if( Boolean( agent.match(new RegExp( userAgents[ i ] , "i")) ) )
				return true;
		}
		return false;
	}
