...
//驗證資料
		...
		if ($DataKey != '') {
			$Sql = " Select Enabled From Personnel where Id = $DataKey ";
			$Focus = $MySql -> db_query($Sql) or die("查詢 Query 錯誤");
			$rs = $MySql -> db_fetch_array($Focus);
			
//更新狀態
			if ($rs["Enabled"] != "Y"){
				$Sql = " Update Personnel Set Enabled = 'Y' Where Id = $DataKey ";
				$Focus1 = $MySql -> db_query($Sql) or die("查詢 Query 錯誤1");
				$rs1 = $MySql -> db_fetch_array($Focus1);
			}else{
				$Sql = " Update Personnel Set Enabled = 'N' Where Id = $DataKey ";	
