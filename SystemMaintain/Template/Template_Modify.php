...
<title>擴思訊 Cross Think 後台 課表範本管理 編輯課表範本</title>
...
<?php include_once(root_path . "/SystemMaintain/CommonPage/header_topmenu_item_5.php");?>        
        ...
        <div class="breadCrumb">
            <ul>
                ...
                <li class="current"><span class="hor-box-text">編輯課表範本</span></li><!-- 如果沒有，就不寫入這個LI -->
        ...
        <form name="DataForm" id="DataForm" method="post" action="Template_Update.php" autocomplete="off" enctype="multipart/form-data">
        	...
        	<table cellpadding="0" cellspacing="0" class="leftHeader import">
            	<tr>
                    ...
                    <td valign="middle"><input type="text" id="TemplateName" name="TemplateName" class="name sized-text-normal" value="<?php echo $rs["TemplateName"];?>"></td>
                </tr>
                <tr>
                	<th valign="top"><span class="hor-box-text-normal">建議上課頻率：</span></th>
                    <td valign="middle">
                    	<?php for($i=0;$i<count($OptionAry);$i++){?>
                        	<label><input type="radio" id="ClassFrequencyId" name="ClassFrequencyId" value="<?php echo $OptionAry[$i][0];?>" <?php if($rs["ClassFrequencyId"]==$OptionAry[$i][0]){?>checked<?php }?>><span class="sized-text-1"><?php echo $OptionAry[$i][4]?></span>&nbsp;</label>
                        <?php }?>
                    </td>
                </tr>
                <tr>
