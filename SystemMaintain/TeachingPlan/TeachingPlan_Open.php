...
//驗證資料
		...
		if ($DataKey != '') {
			$Sql = " Select Enable From TeachingPlan where Id = $DataKey ";
			$Focus = $MySql -> db_query($Sql) or die("查詢 Query 錯誤");
			$rs = $MySql -> db_fetch_array($Focus);
			
//更新狀態
			if ($rs["Enable"] != "Y"){
				$Sql = " Update TeachingPlan Set Enable = 'Y' Where Id = $DataKey ";
				$Focus1 = $MySql -> db_query($Sql) or die("查詢 Query 錯誤1");
				$rs1 = $MySql -> db_fetch_array($Focus1);
			}else{
				$Sql = " Update TeachingPlan Set Enable = 'N' Where Id = $DataKey ";	
