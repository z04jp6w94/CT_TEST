...
//班級資訊		
	$Sql = " select M.Id, ClassName, A.Comment ";
	$Sql .= " from Class M ";
...
//學生資訊
	$Sql = " select M.Id, T.Name, T.NickName from ClassMember M ";
...
//json				
		...
		if($ExcuteSuccess == true){
			...
			for($i=1;$i<=count($Class);$i++){
				...
				$Sql .= " left join ClassMember A on A.StudentId = C.StudentId  ";
				$Sql .= " left join Class B on B.Id = A.ClassId ";
				$Sql .= " where 1 = 1 ";
				$Sql .= " and M.IsRead = 'N' ";
				$Sql .= " and C.PersonnelId = '".$PersonId."' ";
				$Sql .= " and B.Id = '".$Class[$i-1][0]."'";
